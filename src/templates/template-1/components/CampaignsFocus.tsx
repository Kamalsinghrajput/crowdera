  ...
  useEffect(() => {
    if (shapesRef.current) {
      gsap.to(shapesRef.current.children, {
        ...
        scale: 'random(0.9, 1.15)', duration: 'random(2.5, 4.5)',
        repeat: -1, yoyo: true, ease: 'sine.inOut', stagger: 0.4,
      });
    }
  }, []);
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          gsap.fromTo('.campaign-card-item',
            { y: 60, opacity: 0 },
                ...
                >
                  <div className="relative h-44 overflow-hidden">
                    ...
                    <div className="absolute bottom-3 left-3 right-3 z-10">
                      <h3 className="text-white font-extrabold text-sm leading-snug mb-1.5">{c.title}</h3>
                      <span className="bg-brand-yellow text-brand-dark text-[10px] font-bold py-0.5 px-2.5 rounded-full">{c.tag}</span>
                    </div>
                  </div>
                  <div className="p-4 flex flex-col flex-grow">
                    <p className="text-gray-500 text-xs mb-4 flex-grow leading-relaxed">{c.desc}</p>
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-1.5 mb-4 overflow-hidden">
                      <div className="h-1.5 rounded-full bg-gradient-to-r from-brand-yellow to-brand-teal" style={{ width: `${pct}%` }} />
                    </div>
                    <button className="w-full bg-brand-dark text-white font-bold py-2.5 rounded-full hover:bg-brand-teal transition-colors text-xs">
                      Donate Now
                    </button>
                  </div>
                </div>
