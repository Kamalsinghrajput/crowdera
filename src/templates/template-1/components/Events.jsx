    ...
    <section id="events" className="py-20" style={{ background: '#2d3748' }}>
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="flex items-start justify-between mb-10 gap-4 flex-wrap">
          <div>
            <span className="text-brand-yellow font-bold text-sm mb-2 block">Ongoing Events</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-white leading-tight">
              Get Involved: Upcoming<br />Charity Events
            </h2>
          </div>
          <Link href="/templates/template-1/events">
            <a className="inline-flex items-center gap-2 bg-brand-yellow text-brand-dark font-bold px-6 py-3 rounded-full hover:brightness-110 transition-all text-sm shrink-0 mt-2">
          ...
          <div
            ...
            className="bg-white rounded-2xl overflow-hidden flex flex-col sm:flex-row gap-0 shadow-lg hover:shadow-xl transition-shadow">
              ...
              <div className="p-5 flex flex-col justify-between flex-grow">
                <div>
                  <h3 className="font-extrabold text-brand-dark text-base mb-1 leading-snug">{ev.title}</h3>
                  <p className="text-[11px] text-gray-400 font-semibold mb-1">{ev.subtitle}</p>
                  <p className="text-xs text-gray-500 line-clamp-3 mb-3 leading-relaxed">{ev.desc}</p>
                  <div className="flex items-center gap-1.5 text-xs text-brand-teal font-semibold mb-0.5">
                    <MapPin size={12} /> Venue
                  </div>
                  <p className="text-xs text-gray-500 mb-4">{ev.venue}</p>
                </div>
                <Link href={`/templates/template-1/events/${ev.id}`}>
                  <a className="inline-flex items-center gap-2 bg-brand-yellow text-brand-dark font-bold text-xs px-4 py-2 rounded-full hover:brightness-110 transition-all self-start">
