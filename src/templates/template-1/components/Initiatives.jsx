    ...
    <section className="bg-white">
      <div className="bg-[#091F1B] py-20 lg:py-28 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none">
          <div className="absolute top-[-10%] right-[-5%] w-[40%] h-[80%] rounded-full bg-[var(--secondary)] blur-[120px]"></div>
          <div className="absolute bottom-[-10%] left-[-5%] w-[40%] h-[80%] rounded-full bg-[var(--primary)] blur-[120px]"></div>
        </div>
        <div className="container mx-auto px-4 max-w-7xl relative z-10 text-center">
          <span className="text-[var(--primary)] text-xs md:text-sm font-black uppercase tracking-[0.3em] mb-4 block">Make a Difference</span>
          ...
          <div className="max-w-2xl mx-auto relative group">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[var(--secondary)] transition-colors" size={20} />
            <input type="text" placeholder="Search campaigns..." className="w-full pl-16 pr-8 py-5 md:py-6 rounded-full bg-[#FAFAFA] text-gray-900 text-base font-bold shadow-2xl focus:outline-none focus:ring-4 focus:ring-[var(--primary)]/20 transition-all border-none" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} />
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-7xl -mt-10 relative z-20 pb-20">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-8 md:gap-12 border-b border-gray-100">
              {["campaigns", "events", "fundraisers"].map((tabName) => (
                <button key={tabName} onClick={() => handleTabChange(tabName)} className={`pb-4 text-xs md:text-sm font-black uppercase tracking-widest transition-all relative ${activeTab === tabName ? "text-[#091F1B]" : "text-gray-400 hover:text-gray-600"}`}>
                  {tabName}{activeTab === tabName && <div className="absolute bottom-0 left-0 w-full h-1.5 bg-[var(--primary)] rounded-full"></div>}
                </button>
              ))}
            </div>
            <div className="flex flex-wrap items-center gap-4 justify-center sm:justify-start">
              <div className="relative min-w-[140px]">
                <div onClick={() => setIsFilterOpen(!isFilterOpen)} className={`flex items-center justify-between w-full bg-white border px-5 py-3.5 rounded-xl text-sm font-bold text-[#091F1B] transition-all cursor-pointer ${isFilterOpen ? "border-[var(--primary)] ring-2 ring-[var(--primary)]/10" : "border-gray-200 hover:border-[var(--primary)]"}`}><div className="flex items-center gap-2"><Filter size={14} className="text-[var(--secondary)]" /><span>Filter</span></div><ChevronDown size={14} className={`text-gray-400 transition-transform ${isFilterOpen ? "rotate-180" : ""}`} /></div>
                {isFilterOpen && (
                  <><div className="fixed inset-0 z-40" onClick={() => setIsFilterOpen(false)}></div><div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-200">{activeTab === "events" ? ( <div className="space-y-1"><div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-2">Event Options</div><button onClick={() => { setEventFilter("All Events"); setIsFilterOpen(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${eventFilter === "All Events" ? "bg-[var(--secondary)] text-white" : "text-gray-700 hover:bg-gray-50"}`}>All Events</button>{eventFilterOptions.map((filterOption) => ( <button key={filterOption.value} onClick={() => { setEventFilter(filterOption.value); setIsFilterOpen(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${eventFilter === filterOption.value ? "bg-[var(--secondary)] text-white" : "text-gray-700 hover:bg-gray-50"}`}>{filterOption.label}</button> ))}</div> ) : ( <div className="space-y-3"><label className="flex items-center gap-3 cursor-pointer group/item"><input type="checkbox" checked={isVerified} onChange={(e) => setIsVerified(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[var(--secondary)] focus:ring-[var(--secondary)]" /><span className="text-sm font-bold text-gray-700 group-hover/item:text-[var(--secondary)]">Verified</span></label><label className="flex items-center gap-3 cursor-pointer group/item"><input type="checkbox" checked={isTaxExempt} onChange={(e) => setIsTaxExempt(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[var(--secondary)] focus:ring-[var(--secondary)]" /><span className="text-sm font-bold text-gray-700 group-hover/item:text-[var(--secondary)]">Tax Exempt</span></label></div> )}</div></>
                )}
              </div>
              <div className="relative min-w-[160px]"><Tag size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--secondary)]" /><select className="w-full appearance-none bg-white border border-gray-200 pl-10 pr-10 py-3.5 rounded-xl text-sm font-bold text-[#091F1B] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] cursor-pointer" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((categoryItem) => ( <option key={categoryItem} value={categoryItem}>{categoryItem}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div>
              <div className="relative min-w-[160px]"><Globe size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--secondary)]" /><select className="w-full appearance-none bg-white border border-gray-200 pl-10 pr-10 py-3.5 rounded-xl text-sm font-bold text-[#091F1B] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] cursor-pointer" value={country} onChange={(event) => setCountry(event.target.value)}>{countries.map((countryItem) => ( <option key={countryItem} value={countryItem}>{countryItem}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div>
              {activeTab === "fundraisers" && ( <div className="relative min-w-[160px]"><SlidersHorizontal size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--secondary)]" /><select className="w-full appearance-none bg-white border border-gray-200 pl-10 pr-10 py-3.5 rounded-xl text-sm font-bold text-[#091F1B] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] cursor-pointer" value={fundraiserType} onChange={(event) => setFundraiserType(event.target.value)}>{fundraiserOptions.map((typeOption) => ( <option key={typeOption} value={typeOption}>{typeOption}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div> )}
              <div className="relative min-w-[160px]"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--secondary)] text-xs font-black">AZ</div><select className="w-full appearance-none bg-white border border-gray-200 pl-10 pr-10 py-3.5 rounded-xl text-sm font-bold text-[#091F1B] focus:outline-none focus:ring-2 focus:ring-[var(--primary)] cursor-pointer" value={sortBy} onChange={(event) => setSortBy(event.target.value)}>{sortOptions.map((sortOption) => ( <option key={sortOption} value={sortOption}>{sortOption}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40 gap-4 text-[var(--secondary)]">
          ...
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {activeTab === "campaigns" && (filteredCampaigns.length > 0 ? ( <InitiativesCampaigns data={filteredCampaigns} primaryColor="var(--primary)" secondaryColor="var(--secondary)" /> ) : ( <div className="col-span-full py-20 text-center font-bold text-gray-400">No campaigns found matching your filters.</div> ))}
            ...
            {activeTab === "fundraisers" && (filteredFundraisers.length > 0 ? ( <InitiativesFundraisers data={filteredFundraisers} primaryColor="var(--primary)" secondaryColor="var(--secondary)" /> ) : ( <div className="col-span-full py-20 text-center font-bold text-gray-400">No fundraisers found matching your filters.</div> ))}
          </div>
        )}
      </div>
    </section>
