...
const InitiativesFundraisers = ({ data: initialFundraisersData }) => {
  ...
  return (
    <>
      {fundraisersList.map((fundraiserItem) => {
        ...
        return (
          <div key={fundraiserItem.id} className="bg-white rounded-[2rem] hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] transition-all duration-500 group flex flex-col border-2 border-gray-100 opacity-0 p-6 md:p-8">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="min-w-0">
                <div className="font-black text-[#223632] text-base leading-tight">{fundraiserItem.name}</div>
                <div className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1 truncate">Supporting {fundraiserItem.campaignName}</div>
              </div>
            </div>

            {/* Verified / Tax Exempt Badges */}
            {(fundraiserItem.isVerified || fundraiserItem.isTaxExempt) && (
            ...
            {/* Description — using dark green color */}
            <p className="text-[#223632] text-sm leading-relaxed mb-6 line-clamp-3 font-semibold opacity-80">
              {fundraiserItem.desc}
            </p>
