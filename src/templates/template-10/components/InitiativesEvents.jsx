  ...
  return (
    <>
      {eventsList.map((eventItem) => {
        ...
        return (
          <div key={eventItem.id} className="bg-white rounded-[2.5rem] overflow-hidden hover:shadow-[0_20px_50px_rgba(232,84,122,0.15)] transition-all duration-500 group flex flex-col border border-gray-100 opacity-0">
            ...
            <div className="relative h-64 overflow-hidden">
              ...
              <div className="absolute top-4 left-4 flex gap-2 flex-wrap">
                {eventItem.isVerified && (
                  <span className="bg-[#e8547a] text-white text-[10px] font-bold py-1.5 px-4 rounded-full uppercase tracking-widest shadow-lg">Verified</span>
                )}
                {eventItem.isTaxExempt && (
                  <span className="bg-[#1a1a2e] text-white text-[10px] font-bold py-1.5 px-4 rounded-full uppercase tracking-widest shadow-lg">Tax Exempt</span>
                )}
              </div>
            </div>
            ...
            <div className="p-8 flex flex-col flex-grow text-left">
              ...
              <div className="flex items-center gap-1.5 mb-5">
                ...
                <span className="text-xs text-[#e8547a] font-bold uppercase tracking-widest">{eventItem.organizer}</span>
              </div>

              {/* Date / Time Box */}
              <div className="flex items-start gap-3 bg-[#fdf0f4] rounded-2xl px-4 py-4 mb-6 border border-[#e8547a]/10">
                <Calendar size={20} className="text-[#e8547a] mt-0.5 shrink-0" />
                <div>
                  <div className="text-xs font-bold text-[#1a1a2e] uppercase tracking-widest">{eventItem.date}</div>
