...
const InitiativesFundraisers = ({ data: initialFundraisersData, primaryColor = "#e8547a" }) => {
  ...
  return (
    <>
      {fundraisersList.map((fundraiserItem) => {
        ...
        return (
          <div key={fundraiserItem.id} className="bg-white rounded-[2.5rem] hover:shadow-2xl transition-all duration-500 group flex flex-col border border-gray-100 opacity-0 p-8 text-left">

            {/* Profile Avatar + Name + Subtitle */}
            <div className="flex items-center gap-4 mb-8">
              <div className="relative w-16 h-16 rounded-full overflow-hidden flex-shrink-0 border-4 border-[#e8547a]/20 shadow-lg group-hover:scale-110 transition-transform duration-500">
                <Image src={fundraiserItem.img} alt={fundraiserItem.name} layout="fill" objectFit="cover" />
              </div>
              <div className="min-w-0">
                <div className="font-bold text-[#1a1a2e] text-2xl leading-tight tracking-tight">{fundraiserItem.name}</div>
            ...
            <div className="flex gap-2 flex-wrap mb-6">
              {fundraiserItem.isVerified && (
                <span className="bg-[#1a1a2e] text-white text-[10px] font-bold py-1.5 px-4 rounded-full uppercase tracking-widest shadow-md">Verified</span>
              )}
              {fundraiserItem.isTaxExempt && (
                <span className="bg-[#e8547a] text-white text-[10px] font-bold py-1.5 px-4 rounded-full uppercase tracking-widest shadow-md">Tax Exempt</span>
              )}
            </div>

            {/* Description — using primary color for consistency with design style */}
            <p className="text-[#e8547a] text-[15px] leading-relaxed mb-8 line-clamp-3 font-bold uppercase tracking-tight italic">
              {fundraiserItem.desc}
            </p>
            ...
            <div className="bg-[#fdf2f5] rounded-[2rem] p-6 mb-8 border border-[#fbd3de]">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[11px] font-black text-[#1a1a2e] uppercase tracking-widest">Progress</span>
                <span className="text-sm font-black text-[#e8547a]">{progressPercentage}%</span>
              </div>
            ...
            <div className="mt-auto grid grid-cols-2 gap-3">
              <button className="bg-[#1a1a2e] text-white font-black py-4 rounded-full text-[10px] uppercase tracking-[0.2em] hover:bg-[#e8547a] transition-all active:scale-95 shadow-lg">
                Donate
              </button>
