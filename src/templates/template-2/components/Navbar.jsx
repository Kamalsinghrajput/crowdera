...
export default function Navbar() {
  ...
  const isSubpage = router.pathname !== "/templates/template-2";
  ...
  const scrollTo = (e, href) => {
    ...
    if (href.startsWith("#")) {
      if (isSubpage) {
        router.push(`/templates/template-2${href}`);
      } else {
    ...
    <>
      ...
      >
        ...
        <nav
          style={{
            background:
              isSubpage || scrolled ? "#121D18" : "rgba(255,255,255,0.05)",
            backdropFilter: isSubpage || scrolled ? "none" : "blur(2px)",
            boxShadow: scrolled ? "0 2px 20px rgba(0,0,0,0.3)" : "none",
            transition: "all 0.4s ease",
          }}
        >
          <div
            className="max-w-[1320px] mx-auto px-3 flex items-center justify-between"
            style={{ gap: 20 }}
          >
            {/* Left: Logo + Links */}
            <div
