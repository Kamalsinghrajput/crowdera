  ...
  return (
    <div className="bg-white rounded-[20px] overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-xl transition-all duration-300 group relative z-0">
      ...
      <div className="w-full md:w-[40%] relative min-h-[250px] overflow-hidden flex-shrink-0 z-10">
        ...
        />
        {/* Date badge on image */}
        <div className="absolute top-4 left-4 bg-[var(--secondary)] text-white font-['Montserrat'] font-bold text-[12px] uppercase px-3 py-1 rounded">
          {blogData.date}
        </div>
      </div>
      ...
      <div className="w-full md:w-[60%] p-8 md:p-10 flex flex-col justify-center z-10">
        ...
        <Link href="/templates/template-5/blog">
          <a className="font-['Montserrat'] font-extrabold text-[24px] leading-[1.3] text-[var(--bg-color)] group-hover:text-white transition-colors duration-500 line-clamp-2 mb-4 no-underline">
            {blogData.title}
          </a>
        </Link>
