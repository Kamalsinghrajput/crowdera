...
export default function NewsLetter() {
  ...
  const secondaryBgColor = "var(--secondary-bg-color)";
  ...
  return (
    <section className="relative pt-[120px] pb-[80px] overflow-hidden bg-[var(--bg-color)]">
      ...
      {/* Gradient overlay to blend with adjacent sections seamlessly */}
      <div 
        className="absolute inset-0" 
        style={{
          background: "linear-gradient(to bottom, var(--bg-color) 0%, color-mix(in srgb, var(--bg-color) 40%, transparent) 20%, color-mix(in srgb, var(--bg-color) 40%, transparent) 80%, var(--bg-color) 100%)"
        }}
      />
      ...
      <div className="max-w-[800px] mx-auto px-4 relative z-10 text-center">
        ...
        >
          <input
            ...
            required
            className="w-full bg-transparent border border-white/20 rounded-full px-8 py-[18px] text-[17px] font-['Inter'] text-white placeholder:text-white/40 focus:outline-none focus:border-[var(--secondary)] transition-colors pr-[70px]"
          />
          <button
            type="submit"
