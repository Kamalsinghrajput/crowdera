...
const Initiatives = ({ initialTab = "campaigns" }) => {
  ...
  return (
    <section className="bg-white">
      {/* Template 8 Specific Header - Green with Overlay */}
      <div className="bg-[#00715D] py-24 lg:py-32 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] pointer-events-none"></div>
        <div className="container mx-auto px-4 max-w-[1200px] relative z-10 text-center">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 tracking-tight">Our Initiatives</h1>
            <p className="text-white/80 text-lg mb-10 max-w-2xl mx-auto">Help us empower communities and create lasting impact through our diverse range of campaigns and events.</p>
          <div className="max-w-2xl mx-auto relative group">
            ...
            <input type="text" placeholder="Search initiatives..." className="w-full pl-16 pr-8 py-5 rounded-full bg-white text-gray-900 text-base font-medium shadow-2xl focus:outline-none focus:ring-4 focus:ring-[#D9A86A]/30 transition-all border-none" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} />
          </div>
        </div>
      </div>

      <div className="container mx-auto px-4 max-w-[1200px] py-20 relative z-20">
        <div className="bg-white rounded-2xl p-6 md:p-8 mb-16 shadow-xl border border-gray-100">
          <div className="flex flex-col gap-10">
            <div className="flex items-center justify-center sm:justify-start gap-10 md:gap-14 border-b border-gray-100 overflow-x-auto no-scrollbar">
              {["campaigns", "events", "fundraisers"].map((tabName) => (
                <button key={tabName} onClick={() => handleTabChange(tabName)} className={`pb-5 text-sm font-bold uppercase tracking-widest transition-all relative whitespace-nowrap ${activeTab === tabName ? "text-[#00715D]" : "text-gray-400 hover:text-[#00715D]"}`}>
                  {tabName}{activeTab === tabName && <div className="absolute bottom-0 left-0 w-full h-1 bg-[#D9A86A]"></div>}
                </button>
              ))}
            </div>
            <div className="flex flex-wrap items-center gap-4 justify-center sm:justify-start">
              <div className="relative min-w-[140px]">
                <div onClick={() => setIsFilterOpen(!isFilterOpen)} className={`flex items-center justify-between w-full bg-white border px-5 py-3.5 rounded-full text-sm font-bold text-[#111] transition-all cursor-pointer ${isFilterOpen ? "border-[#00715D]" : "border-gray-200 hover:border-[#00715D]"}`}><div className="flex items-center gap-2"><Filter size={14} className="text-[#00715D]" /><span>Filter</span></div><ChevronDown size={14} className={`text-gray-400 transition-transform ${isFilterOpen ? "rotate-180" : ""}`} /></div>
                {isFilterOpen && (
                  <><div className="fixed inset-0 z-40" onClick={() => setIsFilterOpen(false)}></div><div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 p-4 z-50 animate-in fade-in slide-in-from-top-2 duration-200">{activeTab === "events" ? ( <div className="space-y-1"><div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 px-2">Event Options</div><button onClick={() => { setEventFilter("All Events"); setIsFilterOpen(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${eventFilter === "All Events" ? "bg-[#00715D] text-white" : "text-gray-700 hover:bg-gray-50"}`}>All Events</button>{eventFilterOptions.map((filterOption) => ( <button key={filterOption.value} onClick={() => { setEventFilter(filterOption.value); setIsFilterOpen(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-bold transition-colors ${eventFilter === filterOption.value ? "bg-[#00715D] text-white" : "text-gray-700 hover:bg-gray-50"}`}>{filterOption.label}</button> ))}</div> ) : ( <div className="space-y-3"><label className="flex items-center gap-3 cursor-pointer group/item"><input type="checkbox" checked={isVerified} onChange={(e) => setIsVerified(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[#00715D] focus:ring-[#00715D]" /><span className="text-sm font-bold text-gray-700 group-hover/item:text-[#00715D]">Verified</span></label><label className="flex items-center gap-3 cursor-pointer group/item"><input type="checkbox" checked={isTaxExempt} onChange={(e) => setIsTaxExempt(e.target.checked)} className="w-5 h-5 rounded border-gray-300 text-[#00715D] focus:ring-[#00715D]" /><span className="text-sm font-bold text-gray-700 group-hover/item:text-[#00715D]">Tax Exempt</span></label></div> )}</div></>
                )}
              </div>
              <div className="relative min-w-[160px]"><Tag size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-[#00715D]" /><select className="w-full appearance-none bg-white border border-gray-200 pl-12 pr-10 py-3.5 rounded-full text-sm font-bold text-[#111] focus:outline-none focus:ring-2 focus:ring-[#00715D] cursor-pointer" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((categoryItem) => ( <option key={categoryItem} value={categoryItem}>{categoryItem}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div>
              <div className="relative min-w-[160px]"><Globe size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-[#00715D]" /><select className="w-full appearance-none bg-white border border-gray-200 pl-12 pr-10 py-3.5 rounded-full text-sm font-bold text-[#111] focus:outline-none focus:ring-2 focus:ring-[#00715D] cursor-pointer" value={country} onChange={(event) => setCountry(event.target.value)}>{countries.map((countryItem) => ( <option key={countryItem} value={countryItem}>{countryItem}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div>
              {activeTab === "fundraisers" && ( <div className="relative min-w-[160px]"><SlidersHorizontal size={14} className="absolute left-6 top-1/2 -translate-y-1/2 text-[#00715D]" /><select className="w-full appearance-none bg-white border border-gray-200 pl-12 pr-10 py-3.5 rounded-full text-sm font-bold text-[#111] focus:outline-none focus:ring-2 focus:ring-[#00715D] cursor-pointer" value={fundraiserType} onChange={(event) => setFundraiserType(event.target.value)}>{fundraiserOptions.map((typeOption) => ( <option key={typeOption} value={typeOption}>{typeOption}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div> )}
              <div className="relative min-w-[160px]"><div className="absolute left-6 top-1/2 -translate-y-1/2 text-[#00715D] text-xs font-black">AZ</div><select className="w-full appearance-none bg-white border border-gray-200 pl-12 pr-10 py-3.5 rounded-full text-sm font-bold text-[#111] focus:outline-none focus:ring-2 focus:ring-[#00715D] cursor-pointer" value={sortBy} onChange={(event) => setSortBy(event.target.value)}>{sortOptions.map((sortOption) => ( <option key={sortOption} value={sortOption}>{sortOption}</option> ))}</select><ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16} /></div>
              <button onClick={handleReset} className="flex items-center justify-center gap-2 px-8 py-3.5 rounded-full bg-gray-50 hover:bg-gray-100 text-[#111] font-bold text-sm transition-all shadow-sm"><RefreshCw size={14} /><span>Reset</span></button>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {activeTab === "campaigns" && (filteredCampaigns.length > 0 ? ( <InitiativesCampaigns data={filteredCampaigns} primaryColor="#00715D" secondaryColor="#D9A86A" /> ) : ( <div className="col-span-full py-24 text-center font-bold text-gray-400">No campaigns found matching your search.</div> ))}
            ...
            {activeTab === "fundraisers" && (filteredFundraisers.length > 0 ? ( <InitiativesFundraisers data={filteredFundraisers} primaryColor="#00715D" secondaryColor="#D9A86A" /> ) : ( <div className="col-span-full py-24 text-center font-bold text-gray-400">No fundraisers found matching your search.</div> ))}
          </div>
        )}
      </div>
    </section>
